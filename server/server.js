...
app.get('/getStudents', (req, res) => {
    ...
    // Devuelve todos los alumnos cntenidos en la base de datos
    connect.then(conn => {
        conn.query(`select nombre,apellidos from asistencias group by nombre`)
            .then(rows => {
                if(rows.length > 0) {
                    res.json({Students: rows})                 
                }else {
                    res.json({ error: "No se ha registrado ningun estudiante" })
                }
            })
    })
});
