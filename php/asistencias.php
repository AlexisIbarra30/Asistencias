    ...
    switch($_SERVER['REQUEST_METHOD']){
        case 'POST':
            ...
            foreach($_POST as $reg){
                ...
                if(isset($reg['id']) and isset($reg['nombre']) and isset($reg['apellidos']) and isset($reg['fecha']) and isset($reg['hora_entrada'])and isset($reg['hora_salida']) and isset($reg['horas_permanencia'])){
                    ...
                    //Generamos consulta
                    $query ="SELECT * from asistencias2 WHERE nombre='".$reg['nombre']."' and apellidos='".$reg['apellidos']."' and fecha='".$reg['fecha']."' and hora_entrada='".$reg['hora_entrada']."' and hora_salida='".$reg['hora_salida']."'";
                    $res = mysqli_query($con,$query);
                    if(mysqli_num_rows($res)==0){
                        //El registro no existe, lo inserta
                        $query="INSERT INTO asistencias2(nombre,apellidos,fecha,hora_entrada,hora_salida,horas_permanencia,programa)";
        ...
        case 'GET':
            ...
            if(!(isset($_GET['fecha_inicio']) and isset($_GET['fecha_fin']) ))
                $query="SELECT nombre,apellidos,fecha,hora_entrada,hora_salida,horas_permanencia FROM asistencias2 order by fecha desc";
            else    //Devuelve solo dentro de un rango de fechas
                $query="SELECT nombre,apellidos,fecha,hora_entrada,hora_salida,horas_permanencia FROM asistencias2 WHERE fecha BETWEEN '".$_GET['fecha_inicio']."' and '".$_GET['fecha_fin']."'";
